...
test("the CLI still runs when invoked through a symlinked path", async () => {
  ...
  const target = join(projectRoot(), "scripts");
...
test("Windows resolves shell commands using the .cmd extension", () => {
  assert.equal(resolveCommandForPlatform("vite", "win32"), "vite.cmd");
  assert.equal(resolveCommandForPlatform("npm", "win32"), "npm.cmd");
  assert.equal(resolveCommandForPlatform("vite", "linux"), "vite");
  assert.equal(resolveCommandForPlatform("./node_modules/.bin/vite", "win32"), "./node_modules/.bin/vite");
});
